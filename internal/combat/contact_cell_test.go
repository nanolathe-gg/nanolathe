...
func newContactFixture(t *testing.T) (*units.World, *world.Terrain) {
	t.Helper()
	ter := &world.Terrain{CellW: contactCellsPerSide, CellH: contactCellsPerSide}
...
func TestContactLadderRunsUnitSlotsBeforeFeatures(t *testing.T) {
	...
	hitUnit, hitFeature, isWater, isOffMap, bounce := checkCollision(p, wu1913Weapon(10), w, ter, nil)
	if hitUnit != h {
	...
	if _, feature, _, _, _ := checkCollision(p2, wu1913Weapon(10), w, ter, nil); feature == nil {
...
func TestContactDrawsNoRandomness(t *testing.T) {
	...
	if got, _, _, _, _ := checkCollision(p, wu1913Weapon(10), w, ter, nil); got != h {
