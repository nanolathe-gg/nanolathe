...
// Only the left button pairs; the right button keeps its plain press identity.
func TestRightPressesNeverPair(t *testing.T) {
	in := input.NewState()
	var clicks doubleClickRecognizer
	right := func(down bool, at uint32) input.PointerEvent {
		applyInputWith(in, sampledInput{x: 40, y: 50, timestamp: at, buttons: input.MouseButtons{Right: down}}, &clicks)
		event, ok := in.CurrentPointer()
		if !ok {
			t.Fatalf("no published record at t=%d", at)
		}
		return event
	}
	right(true, 100)
	right(false, 100)
	if got := right(true, 101); got.Kind != input.RightDown {
		t.Fatalf("second right press = %v, want RightDown", got.Kind)
	}
}
