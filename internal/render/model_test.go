...
// pieceDraws runs the one per-piece traversal on a fresh scratch slot and keeps
// only the draw records, which is all these ordering and geometry checks read.
func pieceDraws(m *model.Model, states []model.PieceState, worldPos [3]numeric.Fixed, dirty bool) []PieceDraw {
	out, _ := buildPieceDrawsInto(m, states, worldPos, dirty, true, &DrawScratch{})
	return out
}

// projectilePieceStates folds projectile yaw/pitch into a fresh state slice the
// way the standalone projectile piece builder does [03 §5.2].
func projectilePieceStates(m *model.Model, base []model.PieceState, yaw, pitch uint16) []model.PieceState {
	out := make([]model.PieceState, len(m.Pieces))
	copy(out, base)
	FoldProjectileAngles(out, m.Root, yaw, pitch)
	return out
}
...
func TestUnitOrientationFoldOrder(t *testing.T) {
	...
	states := BuildUnitDrawInto(m, nil, 16384, 0, 0, frame.UnitView{}, nil, &DrawScratch{}).PieceStates // heading 16384→Y, pitch 0→X, bank 0→Z [03 §2.4] C24
	...
	// Verify building the piece draws includes position only at final placement [03 §2.4] C24 [03 §5.2]
	// lerpPos zero -> worldOrigin == localOrigin
	pieces := pieceDraws(m, states, [3]numeric.Fixed{}, false)
	...
	pieces2 := pieceDraws(m, states, lerpPos, false)
	...
	states2 := BuildUnitDrawInto(m2, nil, 0, 16384, 0, frame.UnitView{}, nil, &DrawScratch{}).PieceStates // pitch 16384→X
	...
	states3 := BuildUnitDrawInto(m, nil, 0, 0, 16384, frame.UnitView{}, nil, &DrawScratch{}).PieceStates // bank 16384→Z
	...
	// BuildUnitDrawInto samples position and angles from the committed current view [03 §2.4].
	cur := frame.UnitView{Slot: 1, X: numeric.Fixed(65536), Heading: 16384, Pitch: 16384, Bank: 0}
	draw := BuildUnitDrawInto(m, nil, cur.Heading, cur.Pitch, cur.Bank, cur, nil, &DrawScratch{})
	if draw == nil {
		t.Fatal("BuildUnitDrawInto nil")
	}
...
func TestProjectileOffsetWrap(t *testing.T) {
	...
	// The same fold over a whole state slice carries the offset [03 §5.2].
	ps := projectilePieceStates(m, nil, 1000, 2000)
	if ps[0].RotY != 33768 || ps[0].RotX != 34768 {
		t.Fatalf("projectile piece states: got Y=%d X=%d", ps[0].RotY, ps[0].RotX)
	}
	...
	yaw90 := uint16(16384)
	pitch0 := uint16(0)
	ps2 := projectilePieceStates(m, nil, yaw90, pitch0)
	...
	// ps2 is one piece long; mProj needs two, so fold again over its own model.
	_ = ps2
	ps3 := projectilePieceStates(mProj, nil, yaw90, pitch0)
...
func TestPieceDrawOrder(t *testing.T) {
	...
	// Ensure the reordering at load is not redone: the piece draws must output in piece.Primitives order [03 §2.4] C20
	...
	pieces2 := pieceDraws(m, states, [3]numeric.Fixed{}, false)
	for i := range prims {
	...
	tables.Logical[5] = 42
	r, g, b, a := tables.RGBA(5) // [03 §4.3] C10
	if r != 5 || g != 255-5 || b != 2 {
		t.Fatalf("direct palette lookup: got %d %d %d want 5 %d 2", r, g, b, 255-5)
	}
	if a != 255 {
		t.Fatalf("alpha")
	}
...
func TestLeafAttachmentEmit(t *testing.T) {
	...
	// A piece that carries vertices and children is not a strict leaf
	// attachment, and its child still is [03 §2.4] C23.
	...
	pieces2 := pieceDraws(m2, states2, lerpPos, false)
	if pieces2[1].IsLeafAttachment {
		t.Fatalf("nonleaf with children should not be strict leaf")
	}
	if !pieces2[2].IsLeafAttachment {
		t.Fatalf("leaf2 should be leaf")
	}
	// The leaf's world vertex is its composed local point plus the draw
	// position, which is what a consumer of the attachment reads [03 §2.4] C21.
	tr := model.Compose(m2, states2, 2)
	local := tr.Apply(m2.Pieces[2].Vertices[0])
	want := [3]numeric.Fixed{local[0].Add(lerpPos[0]), local[1].Add(lerpPos[1]), local[2].Add(lerpPos[2])}
	if got := pieces2[2].WorldVertices[0]; got != want {
		t.Fatalf("leaf world vertex = %v, want %v", got, want)
	}
}
...
func TestBuildPieceDrawsSuppressesHiddenAncestors(t *testing.T) {
	...
	draws := pieceDraws(m, states, [3]numeric.Fixed{}, false)
	if len(draws) != 2 {
...
func TestBuildPieceDrawsBoundsCyclicParentWalk(t *testing.T) {
	...
	draws := pieceDraws(m, nil, [3]numeric.Fixed{}, false)
	if len(draws) != 2 {
		t.Fatalf("piece count %d want 2", len(draws))
	}
...
// TestWorldPointProjectionOrthographicFormula locks the world-point projection
// every layer in this package places geometry with [03 §2.5]:
// screenX = (worldX>>16) - cameraX + 128 and
// screenY = (worldZ>>16) - ((worldY>>16)>>1) - cameraZ + 32. The half-height
// shear is the part that goes wrong silently.
func TestWorldPointProjectionOrthographicFormula(t *testing.T) {
	cam := &camera.Camera{X: 2, Z: 3}
	world := [3]numeric.Fixed{numeric.Fixed(10 * 65536), numeric.Fixed(5 * 65536), numeric.Fixed(20 * 65536)}
	sx, sy := cam.WorldToScreen(world[0], world[1], world[2])
	if wantX, wantY := int32(10-2+128), int32(20-(5>>1)-3+32); sx != wantX || sy != wantY {
		t.Fatalf("projection = %d,%d, want %d,%d [03 §2.5]", sx, sy, wantX, wantY)
	}
	// A negative height shears up-screen rather than truncating toward zero.
	world[1] = numeric.Fixed(-5 * 65536)
	if _, sy = cam.WorldToScreen(world[0], world[1], world[2]); sy != int32(20-(-5>>1)-3+32) {
		t.Fatalf("negative-height shear = %d, want %d [03 §2.5]", sy, int32(20-(-5>>1)-3+32))
	}
}
