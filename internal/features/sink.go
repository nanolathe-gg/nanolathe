...
// sinkTick implements the wreck sinking state machine [05 "Feature sinking and water interaction"].
// It is called every feature-phase tick.
func (s *Service) sinkTick() {
	if s.Terrain == nil {
		return
	}
	for _, idx := range s.sortedInstanceKeys() {
		inst := s.instances[idx]
		if inst == nil || !inst.IsSinking || inst.Settled {
			continue
		}
		...
		// (fraction discarded), velocities zero, next tick moves to dormant [05 ...].
		if inst.Y.Raw() <= floor.Raw() {
			// Fraction discarded: snap to integer floor height.
			inst.Y = numeric.Fixed(int64(floor.Int()) * 65536)
			inst.Vy = 0
			// Mark settled; next tick's settled test would move to dormant list.
			// For our implementation, settled instances stay but are not sunk further.
			inst.Settled = true
