...
func (q *Queue) Snapshot() (primary []*Node, secondary []*Node) {
	...
	// Copy both the slices and node values. Save capture is a publication
	// boundary: retaining the authoritative node pointers here would let a
	// subsequent pump mutate the purported snapshot while it is being
	// translated to save records [I6][P0-I11].
	return cloneNodes(q.primary), cloneNodes(q.secondary)
}
...
func (q *Queue) RestoreSnapshot(primary, secondary []*Node) {
	...
	// Restore owns its queue nodes. Do not retain pointers from a decoded
	// record or caller-owned staging slice; this keeps subsequent queue pumps
	// from changing the caller's snapshot and vice versa [I6][P0-I11].
	q.primary = cloneNodes(primary)
	q.secondary = cloneNodes(secondary)
}
...
func (q *Queue) SecondarySnapshot() []*Node {
	...
	return cloneNodes(q.secondary)
}

// cloneNodes makes a value-owned copy of an order chain while preserving its
// producer order and nil slots. Nil slots are not expected in an authoritative
// queue, but retaining them keeps save/debug snapshots lossless for malformed
// or partially restored input.
