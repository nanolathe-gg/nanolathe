...
// TakeWorkSlots is the fixed all-slot weapon call of five ground work handlers:
// `HelpBuild` phase 1, `Capture` phase 0, `ReclaimUnit` phase 0,
// `RepairUnit` phase 1's in-reach arm, and `MobileBuild` phase 1 after a legal
// placement check [04 R-ORD-01 §5]. It is exported for that last site alone,
// because the ground placement visit lives in internal/construction. Retail
// releases all three slots there, which takes them from autonomy until the
// record's destructor hands them back [04 R-ORD-01 §7]. The rule set may
// select the inhibit verb at the same site with the same argument, which is
// how the ProTA 4.8 package lets a builder's weapons keep acquiring while it
// works (research/extensions/prota-engine.md "Weapons acquire targets while
// working"). `SelfRepair`, `RepairUnitNoMove` and the VTOL twins' shared
// preamble keep the release in every mode, and `VTOL_MobileBuild` has no
// placement-time call at all: its takeoff preamble already took the slots
// [04 R-ORD-02 §2].
