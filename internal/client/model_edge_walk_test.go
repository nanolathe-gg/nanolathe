...
// TestConvexQuadPaintsTheSamePixelsAsTheBarycentricFiller locks that only the
// folded cases changed. A convex, front-facing quad has to compose the same
// pixel set through the two-chain walk as it did through the fan triangulation
// and the point-in-triangle test — the old filler is still in the tree for the
// shadow pass, so the comparison is a real one and not a recorded census.
//
// The two rules agree on a convex face's interior but not on its far edges: the
// walk's spans are `[xl, xr)` and its rows `[yTop, yBottom)`, while the
// barycentric test admits every pixel whose barycentrics are non-negative,
// which includes the closing edge. So the relationship asserted is containment
// plus a bounded difference confined to that boundary, not equality of the two
// sets — the boundary rule is itself part of [R-RAST-01 §1] step 5.
func TestConvexQuadPaintsTheSamePixelsAsTheBarycentricFiller(t *testing.T) {
	const w, h = 32, 32
	corners := [][2]int32{{6, 4}, {24, 4}, {24, 22}, {6, 22}}

	p := walkPoly(corners, nil)
	walk := paintedRows(t, w, h, &p, 77)

	// The same quad through the retained fan triangulation and barycentric
	// filler, which is what the shadow rasterization still uses.
	c := &Client{width: w, height: h, indexed: make([]uint8, w*h)}
	img := newModelImage(w, h, 0, 0, 0, 0, true, 1)
	for k := 1; k+1 < len(corners); k++ {
		tri := screenTri{
			x: [3]int32{corners[0][0], corners[k][0], corners[k+1][0]},
			y: [3]int32{corners[0][1], corners[k][1], corners[k+1][1]},
		}
		c.fillTriTarget(img, &tri, 77)
	}

	walkSet := map[int32]bool{}
	for row, cols := range walk {
		for _, col := range cols {
			walkSet[row*w+col] = true
		}
	}
	baryCount, missing := 0, 0
	for i, covered := range img.covered {
		if !covered {
			continue
		}
		baryCount++
		if !walkSet[int32(i)] {
			...
			// Every disagreement must be on the quad's closing edge: the last
			// column or the last row.
			if x != 24 && y != 22 {
				t.Fatalf("walk missed interior pixel (%d,%d) that the barycentric filler painted", x, y)
			}
		}
	}
	if baryCount == 0 {
		t.Fatal("the barycentric filler painted nothing; the fixture is wrong")
	}
	for i := range walkSet {
		if !img.covered[i] {
			t.Fatalf("walk painted pixel (%d,%d) that the barycentric filler did not", i%w, i/w)
		}
	}
	// The interior must dominate: the difference is one edge of an 18x18 quad.
	if missing*4 > baryCount {
		t.Fatalf("walk dropped %d of %d pixels; a convex face should differ only along its closing edge", missing, baryCount)
	}
}
