...
// RS-10 focused tests: mobile/factory lifecycle and save ownership [RS-10].
...
// TestRS10_SaveMidConstruction verifies save mid-mobile-build and mid-factory-production resume exact [RS-10].
func TestRS10_SaveMidConstruction(t *testing.T) {
	// Mid-mobile-build
	cat := &content.Catalog{Units: map[string]*content.UnitDef{}}
	...
	w := units.New(20, cat)
	econ := &economy.Service{}
	hb, _ := w.Create(builderDef, 0, world.CellToWorld(1), 0, world.CellToWorld(1))
	builder := w.Unit(hb)
	builder.Def = builderDef
	siteX := world.CellToWorld(10)
	siteZ := world.CellToWorld(10)
	...
	prod := w.Unit(prodHandle)
	if prod == nil {
		t.Fatalf("prod nil")
	}
	// Now prod has Remaining=1, we are in State3 next tick. Set fractional remaining and blocked deadline simulation?
	// Advance one work step to have fractional remaining 0.7
	prod.Remaining = 0.7
	prod.Health = 60
	node.Phase = uint8(State3)
	node.DynamicGate = WakeBit1 | WakeBit3
	node.Deadline = int32(50)
	// Capture via save.StateV1
	st := &save.StateV1{
		Version: save.StateV1VersionConst,
	}
	// Simulate capture of builder link
	links := svc.SnapshotLinks()
	...
	// Also capture Orders and Units via manual (simplified)
	// Units
	st.Units = []save.UnitRecord{
		{Slot: int32(builder.Handle), DefName: "armck", Owner: builder.Owner, X: int32(builder.X.Raw()), Y: int32(builder.Y.Raw()), Z: int32(builder.Z.Raw()), Health: builder.Health, Remaining: builder.Remaining, Flags: builder.Flags, MaxHealth: builder.MaxHealth},
		{Slot: int32(prod.Handle), DefName: "armllt", Owner: prod.Owner, X: int32(prod.X.Raw()), Y: int32(prod.Y.Raw()), Z: int32(prod.Z.Raw()), Health: prod.Health, Remaining: prod.Remaining, Flags: prod.Flags, MaxHealth: prod.MaxHealth},
	}
	// Orders: capture pending site, queue op state, blocked retry deadline
	st.Orders = []save.OrderRecord{
		{
			UnitSlot: int32(builder.Handle), Segment: 0, Index: 0,
			Descriptor: "MobileBuild", Phase: node.Phase, DynamicGate: node.DynamicGate, Deadline: node.Deadline,
			GoalX: int32(node.GoalX.Raw()), GoalZ: int32(node.GoalZ.Raw()), Target: int32(node.Target),
			Param1: node.Param1, Param2: node.Param2, BuildDefKey: node.BuildDefKey, Flags: node.Flags,
		},
	}
	// Marshal/unmarshal round-trip
	data := save.MarshalStateV1(st)
	decoded, err := save.UnmarshalStateV1(data, "", "")
	if err != nil {
		t.Fatalf("UnmarshalStateV1: %v", err)
	}
	if len(decoded.Construction.BuilderLinks) != 1 {
		t.Fatalf("builder links not persisted, got %d", len(decoded.Construction.BuilderLinks))
	}
	if decoded.Construction.BuilderLinks[0].Builder != int32(builder.Handle) || decoded.Construction.BuilderLinks[0].Product != int32(prod.Handle) {
		t.Fatalf("builder link mismatch %+v", decoded.Construction.BuilderLinks[0])
	}
	if len(decoded.Orders) != 1 {
		t.Fatalf("orders not persisted")
	}
	ord := decoded.Orders[0]
	if ord.GoalX != int32(siteX.Raw()) || ord.GoalZ != int32(siteZ.Raw()) {
		t.Fatalf("pending site not persisted: got %d,%d want %d,%d", ord.GoalX, ord.GoalZ, siteX.Raw(), siteZ.Raw())
	}
	if ord.Phase != uint8(State3) {
		t.Fatalf("queue operation state not persisted")
	}
	if ord.Deadline != int32(50) {
		t.Fatalf("blocked retry deadline not persisted")
	}
	// Find fractional remaining
	found := false
	for _, u := range decoded.Units {
		if u.Slot == int32(prod.Handle) {
			found = true
			if u.Remaining != 0.7 {
				t.Fatalf("fractional remaining not persisted: %v want 0.7", u.Remaining)
			}
			if u.Health != 60 {
				t.Fatalf("health not persisted")
			}
		}
	}
	if !found {
		t.Fatalf("product unit not in save")
	}

	// Restore without firing hooks: ensure OnRefresh not called
	hookFired := false
	svc2 := NewService(terrain, cat, w, econ)
	svc2.OnRefresh = func(u *units.Unit) { hookFired = true }
	links2 := make([]LinkRecord, 0, len(decoded.Construction.BuilderLinks))
	for _, r := range decoded.Construction.BuilderLinks {
		links2 = append(links2, LinkRecord{Builder: pool.Handle(r.Builder), Product: pool.Handle(r.Product)})
	}
	svc2.RestoreLinks(links2)
	if hookFired {
		t.Fatalf("restore should not fire gameplay hooks")
	}
	if _, ok := svc2.BuilderLink(prodHandle); !ok {
		t.Fatalf("restored link missing")
	}

	// Mid-factory-production with blocked retry deadline
	w2 := units.New(20, cat)
	terrain2 := &world.Terrain{CellW: 10, CellH: 10, Plot: make([]world.PlotCell, 100)}
	for i := range terrain2.Plot {
		terrain2.Plot[i].SetFeature(world.PlotFeatureNone)
	}
	terrain2.Plot[4*10+4].SetOccupantA(1)
	facDef := &content.UnitDef{DefinitionHeader: content.DefinitionHeader{CanonicalKey: "armfac"}, UnitName: "armfac", FootprintX: 2, FootprintZ: 2, YardMap: "oo\n00", Builder: true, MaxDamage: 200, WorkerTime: 300}
	cat.Units[content.CanonicalKey("armfac")] = facDef
	...
	qf.Push(bid, orders.Node{BuildDefKey: "armllt", Param1: 1, Param2: 2, Phase: uint8(State2), Deadline: -1})
	headF := qf.Primary()[0]
	...
	// Save this blocked state
	st2 := &save.StateV1{Version: save.StateV1VersionConst}
	st2.Orders = []save.OrderRecord{
		{UnitSlot: int32(factory.Handle), Segment: 0, Index: 0, Descriptor: "BuildingBuild", Phase: headF.Phase, DynamicGate: headF.DynamicGate, Deadline: headF.Deadline, Param2: headF.Param2, BuildDefKey: headF.BuildDefKey},
	}
	data2 := save.MarshalStateV1(st2)
	decoded2, err := save.UnmarshalStateV1(data2, "", "")
	if err != nil {
		t.Fatalf("Unmarshal2: %v", err)
	}
	if len(decoded2.Orders) != 1 || decoded2.Orders[0].Deadline != int32(115) {
		t.Fatalf("factory blocked retry deadline not persisted, got %+v", decoded2.Orders)
	}
	if decoded2.Orders[0].Param2 != 2 {
		t.Fatalf("queue count not persisted")
	}
	if decoded2.Orders[0].Phase != uint8(State2) {
		t.Fatalf("queue operation state not persisted for factory")
	}
}
