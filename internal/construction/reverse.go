...
func ReverseRefund(targetBucket *float32, refund float32, isSpecial bool, modeSelector int) {
	if targetBucket == nil || refund <= 0 {
		return
	}
	creditConstructionRefund(targetBucket, refund, isSpecial, modeSelector)
}

// creditConstructionRefund preserves the negative double difficulty constants
// through the multiply and subtraction, then stores the accumulator once as
