...
func (s *Session) applySpawnCommand(c HumanSpawnCommand, tick uint32) {
	...
	if !query.Mobile {
		...
		if err != nil {
			say("Cannot spawn: invalid building yard")
			return
		}
	}
	if _, err = s.World.CheckPlacement(query); err != nil {
		say("Cannot spawn: location blocked or unsuitable for this unit")
		return
	}
	...
	for cz := rect.MinZ(); cz < rect.MaxZ(); cz++ {
		for cx := rect.MinX(); cx < rect.MaxX(); cx++ {
			if s.World.PlotAt(cx, cz).StructureYard() {
				say("Cannot spawn: location overlaps a building yard")
				return
			}
		}
	}
	if !query.Mobile {
		...
		if _, err = s.World.CheckPlacement(query); err != nil {
			say("Cannot spawn: location blocked")
			return
		}
	}
