...
var disabledGates = map[string]string{
	"cmd/nanolathe/production_o3_test.go::TestProductionInputARMEconomyBuildReplay":             "the authored build chain now completes end to end: armsolar, armmex and armlab all finish and the lab produces its authored product. The gate fails later, at the typed activation contract: pressing O over a selected OnOffable structure queues nothing, and dispatching the same activation command directly enqueues one human command that the session consumes without ever flipping Unit.Activated (measured: selectedUnits=1, selection flag set, OnOffable true, direct dispatch err=nil pending=1, then pending=0 and Activated unchanged across four ticks). The handler is in internal/session",
