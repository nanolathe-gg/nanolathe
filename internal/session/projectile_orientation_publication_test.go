...
func TestPublishedProjectileOrientationKeepsIndependentWords(t *testing.T) {
	...
	p.PropellerYaw = numeric.Angle(0x3456)
	...
	v := got.Projectiles[0]
	if v.Roll != 0x3456 || v.MeteorPitch != 0x4567 {
		t.Fatalf("published roll/pitch = %#04x/%#04x, want 0x3456/0x4567", v.Roll, v.MeteorPitch)
	}
	...
	if !v.Propeller || v.ExpiryTick != 99 {
		t.Fatalf("published propeller/deadline = %v/%d, want true/99", v.Propeller, v.ExpiryTick)
	}
}
