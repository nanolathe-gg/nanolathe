...
</style><h1>Strategic icon catalog</h1><p>Vocabulary revision {{.Revision}} · {{len .Rows}} winning definitions. Independently authored Enhanced presentation symbols. Geometry uses the exact GPU atlas masks and bilinear sampling. Each row: 16 / 20 / 24 px; dark terrain unselected, bright terrain selected. Cyan is a sample owner color, pale gold is a sample selection palette color. Basic constructors use one tool; advanced constructors use crossed tools, qualified by authored CONSTR and LEVEL1/LEVEL2 tokens. Other roles have no level marks. Shared art identifies family and capability, not exact unit identity. Nanoframes share their definition's icon; lifecycle appearance belongs to the runtime capture gate.</p><p>{{if .Constructors}}<a href="constructors.png">Constructor comparison PNG</a> · {{end}}<a href="vocabulary.png">Vocabulary PNG</a> · <a href="catalog.png">Full catalog PNG</a> · <a href="audit.json">Machine-readable audit</a></p>
