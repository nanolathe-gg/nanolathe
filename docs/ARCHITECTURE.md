...
| `[F-P1-008]` | presentation-only zoom, in two halves. `camera.Scale` (a `camera.ViewScale` in half steps: 1×, 1.5×, 2×) is the RECORD step the recorder projects at; `camera.Zoom` (a free factor in 1/1024 units) is the LIVE factor the player sees, and it is what the view size, the chrome insets, middle-drag, picking and the minimap rectangle are measured through. In the classic executor the two are always equal and the picture is §14's three steps; in the modern one the wheel moves the factor freely, the executor scales the recorded world by factor/step, and below half scale the strategic view replaces units with markers. Neither is a retail concept, neither changes authoritative state, and neither is saved. Written by F9, the wheel, middle-drag, `--zoom` and the window's resolution default. DESIGN_INTERFACE_HUD_INPUT §3.8 and DESIGN_GPU_RENDERER §14 and §16 |
