...
| `[OW-3-P]` | goal-family wiring between orders and path search: which order produces which goal family (`[04 §7.2]` `[04 §7.4]` `[04 §3.5]`) — point goals by default, the annulus where retail establishes a stand-off, and the rectangle-perimeter and air families deliberately unwired because no established producer exists. DESIGN_MOVEMENT_PATH. (`[OW-3-O]` is the SC22 heading) |
