...
| `[F-P1-008]` | presentation-only zoom. `camera.Scale` (a `camera.ViewScale` in half steps: 1×, 1.5× and 2×) scales the world view, the chrome insets and pointer conversions; it is not a retail concept, changes no authoritative state, and is driven by F9, middle-drag, `--zoom` and the window's resolution default. DESIGN_INTERFACE_HUD_INPUT §3.8 and DESIGN_GPU_RENDERER §14 |
