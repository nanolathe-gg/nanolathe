...
// showRetailMessage opens the authored message window and binds its first
// authored text control. Dynamic wrapping and sizing are not established, so
// no generated controls or guessed geometry are introduced [07 §3].
func (g *gameShell) showRetailMessage(message string) error {
	...
	if g == nil || g.assets == nil || g.assets.message == nil || g.assets.message.window == nil {
		return g.retailMessageError(logical, "an authored MSGBOX window with an active text control")
	}
	// The message box is an authored GUI window. Its exact dynamic wrapping
	// and control binding are not established; retaining the authored geometry
	// avoids inventing fallback dimensions or generated TEXT gadgets [07 §3].
	m := ui.NewPanel(g.assets.message.window)
	if m == nil {
		return g.retailMessageError(logical, "an authored MSGBOX window with an active text control")
	}
	if !m.SetMessage(message) {
		// MSGBOX.GUI without an authored text control cannot present a
		// diagnostic. Retail's dynamic wrapping/binding is unresolved; do not
