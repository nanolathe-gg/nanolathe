...
The engine runs COB on a small stack-based virtual machine. Each unit
instance gets its own VM state (statics, threads); multiple threads per unit
run cooperatively (`start-script` spawns one). The engine calls named entry
points ("callbacks") such as `Create`, `AimPrimary`, `Killed`.

This document covers: the COB container, the VM and instruction set, the
value scaling conventions, the BOS language, and the engine callback
surface.
...
| 0x10 | NumberOfStatics | Count of static variables the unit needs (historically "Unknown_1"). Statics are zero-initialized by the engine; there is no data section. |
...
- The engine finds callbacks by case-insensitive script name
  (`Create`, `AimPrimary`, ...). Anything not recognized is only reachable
  via `call-script`/`start-script` from other scripts.
...
Angles passed *into* callbacks (`heading`, `pitch`) use the same
65536-per-circle convention. Weapon-aim `heading` is relative to the unit
hull: first derive the world heading with TA's `atan2(dx, dz)` (+Z toward +X),
then subtract the unit heading with 16-bit wrap. This is distinct from the
ordinary mathematical `atan2(dz, dx)` convention.
...
16-bit integers. The angular result ports use the same 65,536-units-per-circle
domain as compiled BOS angle constants. These conventions are part of the
host boundary; the retail census establishes the port stack shapes and uses,
but does not independently expose coordinate pack/unpack arithmetic.
...
Authoritative values from the retail `scripts/SFXTYPE.H`. Vector-based
(emitted along the piece's axis): `SFXTYPE_VTOL` = 0, `SFXTYPE_THRUST` = 1,
`SFXTYPE_WAKE1` = 2, `SFXTYPE_WAKE2` = 3, `SFXTYPE_REVERSEWAKE1` = 4,
`SFXTYPE_REVERSEWAKE2` = 5. Point-based (piece origin):
`SFXTYPE_POINTBASED` = 256, `SFXTYPE_WHITESMOKE` = 256|1,
`SFXTYPE_BLACKSMOKE` = 256|2, `SFXTYPE_SUBBUBBLES` = 256|3. (Damage smoke
is computed at runtime by `SMOKEUNIT.H`, which is why smoke types appear in
bytecode as `0x100 | n` expressions rather than constants.)

### Engine callbacks

The engine invokes these by name when present (parameters shown as in the
BOS guide; `piecenum`-style parameters are *outputs* — the script assigns a
piece to them):

| Callback | When / role |
| --- | --- |
| `Create()` | On unit creation (including start of construction). Init statics, hide flares, `start-script SmokeUnit()`. |
| `Activate()` / `Deactivate()` | On/off state changes |
| `StartMoving()` / `StopMoving()` | Movement begins/ends (wakes, walk loops) |
| `MotionControl(moving, aiming, justmoved)` | Continuous locomotion animation (Kbot walking); all three args are booleans. Signature confirmed from Scriptor's own compiler argument-checking table (`COMMON_FUNC` in `Decompiler.cfg`/`Compiler.cfg`), not previously documented here. |
| `SetSpeed(speed)` / `SetDirection(dir)` | Locomotion hints; present in retail scripts (26 and 8 files respectively — hovercraft/ships) though absent from the BOS guide |
| `MoveRate1()` / `MoveRate2()` / `MoveRate3()` | Aircraft speed-class callbacks (retail plane scripts) |
| `AimPrimary(heading, pitch)` `AimSecondary(...)` `AimTertiary(...)` | Aim weapon 1/2/3; return TRUE when aimed. Conventionally `signal SIG_AIM; set-signal-mask SIG_AIM;` first so a new target restarts aiming. |
| `AimFromPrimary(piecenum)` etc. | Which piece the engine measures aim heading/pitch from |
| `QueryPrimary(piecenum)` etc. | Which piece the projectile spawns from (flares; multi-barrel scripts alternate) |
| `FirePrimary()` etc. | Fire animation: recoil, flash show/hide |
| `SetMaxReloadTime(time)` | Called with the weapon reload time (ms); stock scripts store `restore_delay = time * 2` |
| `RestoreAfterDelay()` | Return turrets to rest after `restore_delay` |
| `TargetCleared(which)` / `TargetHeading(heading)` | Target loss / build-target heading |
| `HitByWeapon(anglex, anglez)` | On damage (rock the unit, temporary deactivate) |
| `RockUnit(anglex, anglez)` | Recoil/rock helper callback |
| `SmokeUnit(healthpercent, sleeptime, smoketype)` | Damage smoke loop (authored helper started from `Create`, not an engine entry). Parameter signature confirmed from Scriptor's `COMMON_FUNC` table; all three args are numeric. |
| `QueryNanoPiece(piecenum)` | Piece emitting the nanolathe spray; called repeatedly, stock builders alternate between spray pieces |
| `StartBuilding(...)` / `StopBuilding()` | Build begins/ends. Factories: zero-arg; mobile builders: `(heading)`; commander: `(heading, pitch)` |
| `QueryBuildInfo(piecenum)` | Factory pad piece the product is attached to while built |
| `OpenYard()` / `CloseYard()` | Yardmap open/close (with `set YARD_OPEN to ...`) |
| `QueryLandingPad(piece1, piece2)` | Landing pad pieces for aircraft |
| `QueryTransport(piecenum)` `BeginTransport(height)` `EndTransport()` | Air-transport attach/detach (`ARMATLAS`/`CORVALK`). `CORVALK` declares `QueryTransport(piecenum, height)` — the engine passes a height argument that most scripts simply omit. |
| `TransportPickup(unitid)` / `TransportDrop(unitid, position)` | Sea/hover transport load/unload. Engine-invoked with the cargo's unit ID (and, for drop, the packed x,z destination); the script does the whole crane animation, then `attach-unit`/`drop-unit`. Absent from the BOS guide but present in all 12 retail sea/hover transport scripts. |
| `SweetSpot(piecenum)` | Aim-target piece for enemies |
| `Killed(severity, corpsetype)` | Death animation. `severity` = damage overkill (percent-like); script assigns `corpsetype` out: 1 = leave `_dead` corpse, 2 = `_heap`, 3 = nothing. |
| `Demo()` | Unit-viewer hook, unused in game |
...
- Common helper-script conventions (script names that are not engine
  ...
  `walklegs`/`walkscr`/`stand`/`swim` (Kbot locomotion), `setSFXoccupy`,
...
- Opcode cross-check (behavioral relay only, no code copied): the opcode
...
- Scaling constants verified against the retail
  ...
  OpenTA parser: `internal/sim/subsystem/script/program/`, VM:
  `internal/sim/subsystem/script/runtime/`.
