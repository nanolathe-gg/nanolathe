...
| **3DO model** (§8) | *garbage or fault*: whole-load, then unconditional relocation of the name, vertex, primitive, sibling and child offsets and each primitive's three offsets; nothing is bounded | same | none: the version signature is **never read** | piece names: the script-to-model link pass searches for each script piece from its own slot onward and takes the first match there, so duplicate model names go to successive script entries of that name; a name with no match from its own slot onward keeps the record its slot already holds, and a script piece beyond the model's piece count has no record (`[04 R-COB-01 §4]`) | texture name that no texture GAF holds → the primitive becomes flat colour index 209 (`[03 §2.4]`); the model file missing → **fatal** (path) at unit compile, weapon compile and feature compile alike | *skip*: null → fatal as above | counts drive signed loops; a huge vertex count only walks memory |
| **COB script** (§8) | *garbage or fault*: whole-load, checksum over the file, then relocation of the five tables by their declared counts, unbounded | same | none: the version word is **never read** | script names: linear first match (`[04 R-COB-01 §1]`); piece names: the slot pass of `[04 R-COB-01 §4]`, first match from the script piece's own slot onward, so a repeated script piece name never reclaims the model piece an earlier entry took (the map is one-to-one) | piece name absent from the model → keeps its slot's record; script pieces beyond the model's piece count have no record (`[04 R-COB-01 §4]`); the script file missing → null, and the first unit created from that definition **crashes** (`[04 R-COB-04 §8]`) | *skip*: null → as missing | signed loop bounds; the code array is never bounded (doc 04 owns out-of-range jumps) |
